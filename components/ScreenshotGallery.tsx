...
import ImageModal, { ScreenshotItem } from "@/components/ImageModal";

export default function ScreenshotGallery() {
  const items = useMemo<ScreenshotItem[]>(
    () => [
      {
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {items.map((item, index) => (
          ...
          >
            ...
            <div className="pointer-events-none absolute inset-0 flex items-center justify-center rounded-2xl bg-background/0 text-sm font-semibold text-foreground opacity-0 transition group-hover:bg-background/70 group-hover:opacity-100">
              Click to zoom
            </div>
          </button>
        ))}
